...
if __name__ == '__main__':
    ...
    model = net.Net(net_info["continuous_dim"], net_info["categorical_dim"], net_info["output_dim"], net_info["hidden_dim"], net_info["conv_depth"], train_info["mode"], graph_info["k"], nn.Sigmoid()).to(device)
    ...
    for epoch in range(first_epoch+1,31):
        ...
        training_loss, training_time = train(model, device, optimizer, scheduler, loss_fn, train_dl, epoch)
        ...
        test_metrics, resolutions, evaluation_time = evaluate(model, device, loss_fn, test_dl, metrics, deltaR, deltaR_dz, model_dir, out='', train_info = train_info)
